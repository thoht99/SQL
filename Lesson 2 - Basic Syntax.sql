...
select * from public.region; -- all columns -- 
select REGIONID, REGname from public.region; -- selected columns --
...
select REGname, regionid
from public.region
where public.region.regionid = 1 -- Note: alternative condition can be: in (1,3,4), >= 2, > 2 --
;
...
select regionid,igid, count(cntname)
from public.country
group by regionid, igid;

-- order by with desc--
select regionid,igid, count(cntname)
from public.country
group by regionid, igid
order by regionid desc  -- or asc --
;
...
SELECT CAST('2017-08-25' AS datetime); -- incorrect format
SELECT CAST('2017-08-25' AS datetime); -- correct format
...
Select *, (YearName + INTERVAL '10 year') as plus_10_years
from public.year
;

-- Differences between two datetime points by part of a string--
SELECT DATE_PART('day', '2021-01-01'::date) - DATE_PART('day', '2020-10-02'::date);

Select *, DATE_PART('day', YearName::date) - DATE_PART('day', '2021-10-02'::date) as Date_diff_by_part
from public.year
;

-- How to find number of different date between two point of time --
SELECT ( '2021-01-01'::date) - ('2020-10-02'::date);
Select *,  ('2021-10-02'::date) - (YearName::date)  as Date_diff_by_interval
from  public.year
;
...
select regname,
 concat(regname,'-',regionID) as concat_string,
 lower(regname) as Lower_case,
  --lower(scode) as Lower_case, --
 left(regname,2),
 -- right(scode,2),--
concat('   ',regname) as space_before,
trim(concat('   ',regname)) as Trimped_space,
substring(regname,3,5) as five_strings_from_3,
position('A' in regname) as B_position_in_region
from public.region
;
...
gdp* 2 as double_gdp,
gdp - 10 as minues_10,
gdp / ppnumber  as ratio_gdp_population,
gdp % 2 as remainder
from public.trans;
 ...
 */
select PPtypeID, CountryID, 
count(CountryID) as count_name,
count(CountryID) * 3 as triple_count,
max(GDP) as max_gpd,
min(lifeexpectancy) as min_life,
avg(ppnumber) as average_population
from public.trans
group by  PPtypeID, CountryID
;
...
select igid, igname,
Case
  when igid = 1 then 'High'
  when igid = 2 then 'Low'
  when igid = 3 then 'Lower_middle'
  else 'Upper_middle' end as New_IG
from  public.incomegroup
;
...
select * 
from public.country left join public.region -- right join, inner-join,  outer join --
on public.country.regionID =  public.region.regionID;
...
*/
select tbl1.cntname, tbl1.cntcode, tbl2.regname, tbl3.igname -- *  --
from public.country tbl1 left join public.region tbl2
on tbl1.regionID =  tbl2.regionID
left join public.IncomeGroup tbl3
on tbl1.igid = tbl3.igid
;
...
*/
select * from region where regionID = 2 
UNION
...
left join StockDB_Add.order_hist.trans_order tbl3
    on tbl2.Scode = tbl3.Scode
left join StockDB_Add.trans_frg.ftrans tbl4
    on tbl2.Scode = tbl4.Scode
where StockID = 1
;
...
-- 10) Find average of each Matched transaction. Then, round the result with 2 deceimal number, marking average into  different groups with range like 1) 0 - 100, 101 - 500, and more than 500 times
-- Option 1 --
select 
matchedID,
case
